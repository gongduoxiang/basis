...
public class StatusBarUtil {
     ...
     * @param layout 布局
     */
    public static void setStatusBarHeight(final View layout) {
        if (layout == null) return;
        int statusBarHeight = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            int resourceId = getStatusBarHeight(layout.getContext());
            if (resourceId > 0) {
                statusBarHeight = resourceId;
            }
        } else {
            //低版本 直接设置0
            statusBarHeight = 0;
        }
        layout.setPadding(layout.getPaddingLeft(), statusBarHeight + layout.getPaddingTop(),
        ...
        if (lp.height == 0) {
            layout.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                ...
                public void onGlobalLayout() {
                    lp.height = layout.getHeight() + getStatusBarHeight(layout.getContext());
     ...
     * @param layout 布局
     */
    public static void setStatusBarHeight2(final View layout) {
        int statusBarHeight = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            int resourceId = getStatusBarHeight(layout.getContext());
            if (resourceId > 0) {
     ...
     * @param layout 布局
     */
    public static void setStatusBarHeightMargin(View layout) {
        int statusBarHeight = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            int resourceId = getStatusBarHeight(layout.getContext());
            if (resourceId > 0) {
        ...
        if (layout.getLayoutParams() instanceof LinearLayout.LayoutParams) {
            LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) layout.getLayoutParams();
            lp.setMargins(lp.leftMargin, lp.topMargin + statusBarHeight, lp.rightMargin, lp.bottomMargin);
        } else if (layout.getLayoutParams() instanceof FrameLayout.LayoutParams) {
    ...
    /**
     * 给title设置top
     * android:fitsSystemWindows="true" 时使用
     *
     * @param layout 布局
     */
    public static void setStatusBarHeightMargin2(View layout) {
        int statusBarHeight = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            int resourceId = getStatusBarHeight(layout.getContext());
            if (resourceId > 0) {
    ...
    public static boolean setStatusBarDarkTheme(Activity activity, boolean dark) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                setStatusBarColor(activity, activity.getResources().getColor(R.color.color_55000000));//灰色
            }
        } else {
            setStatusBarColor(activity, activity.getResources().getColor(R.color.color_55000000));//灰色
        }
