...
public class CircularProgressView extends View {
    ...
    private Paint spacing;//进度条中间的间距
    private int spacingW = 8;
    private boolean isSpacing = false;//是否显示间距
    ...
    public CircularProgressView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        // 初始化进度圆环画笔
        spacing = new Paint();
        spacing.setStyle(Paint.Style.STROKE);    // 只描边，不填充
        spacing.setAntiAlias(true);              // 设置抗锯齿
        spacing.setDither(true);                 // 设置抖动
        spacing.setStrokeWidth(typedArray.getDimension(R.styleable.CircularProgressView_progWidth, 10));
        spacing.setColor(Color.WHITE);
        ...
        isSpacing = typedArray.getBoolean(R.styleable.CircularProgressView_isSpacing, false);
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        if (isSpacing) {
            if (mProgress > 50) {//大于一半  把间距算  大的里面
                progress = 360 * mProgress / 100 - spacingW * 2;
                canvas.drawArc(mRectF, 275 + progress + spacingW, 360 - progress - spacingW * 2, false, mBackPaint);
                canvas.drawArc(mRectF, 275, progress, false, mProgPaint);
            } else {
                progress = 360 * mProgress / 100;
                canvas.drawArc(mRectF, 275 + progress + spacingW, 360 - progress - spacingW * 2, false, mBackPaint);
                canvas.drawArc(mRectF, 275, progress, false, mProgPaint);
            }
        } else {
            canvas.drawArc(mRectF, 275, 360, false, mBackPaint);
            canvas.drawArc(mRectF, 275, 360f * mProgress / 100, false, mProgPaint);
        }
    }
